...
const struct test_s test = {
    ...
    .gyro_sens      = 32768/250,
    .accel_sens     = 32768/8,  //FSR = +-2G = 16384 LSB/G
    .reg_rate_div   = 0,    // 1kHz. 
...
//**************mit rev == 1 gibt es Fehler rev == 1 ist unsere Version
    if (rev == 0x1 || rev == 0x3)
...
printf("\ndata mpu_read_mem\n");
    ...
    if (!st.chip_cfg.sensors){
printf("\nst.chip_cfg.sensors mpu_read_mem\n");
        return -1;
}
    ...
    /* Check bank boundaries. */
    if (tmp[1] + length > st.hw->bank_size){
printf("\nCheck bank boundaries mem defined MPU6500\n");
        return -1;
}

    if (i2c_write(st.hw->addr, st.reg->bank_sel, 2, tmp)){
printf("\nread mem defined MPU6500 i2c_write\n");
        return -1;
}
    if (i2c_read(st.hw->addr, st.reg->mem_r_w, length, data)){
printf("\nread mem defined MPU6500 i2c_read\n");
        return -1;
}
    return 0;
}
